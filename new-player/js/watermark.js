(function(){var defaults={clickable:false,url:"",className:'vjs-watermark',text:false,debug:false},extend=function(){var args,target,i,object,property;args=Array.prototype.slice.call(arguments);target=args.shift()||{};for(i in args){object=args[i];for(property in object){if(object.hasOwnProperty(property)){if(typeof object[property]==='object'){target[property]=extend(target[property],object[property]);}else{target[property]=object[property];}}}}
return target;};var div;videojs.registerPlugin('watermark',function(settings){var options,player,video,img,link;options=extend(defaults,settings);player=this.el();video=this.el().getElementsByTagName('video')[0];if(!div){div=document.createElement('div');div.className=options.className;}
else{div.innerHTML='';}
if(options.text)
